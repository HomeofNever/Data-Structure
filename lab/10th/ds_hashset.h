//
// Created by luox6 on 2019/4/2.
//

#ifndef INC_10TH_DS_HASHSET_H
#define INC_10TH_DS_HASHSET_H
...
public:
    ...
    std::pair< iterator, bool > insert(KeyType const& key) {
      ...
      if (p == m_table[index].end()) {
        m_size++;
        p = m_table[index].insert(p, key);

        return std::pair<iterator, bool> (iterator(this, index, p), true);
      } else
        return std::pair<iterator, bool> (iterator(this, index, p), false);
    }
...
private:
    ...
    void resize_table(unsigned int new_size) {
      ...
      std::vector< std::list< KeyType >> enlarge(new_size);

      for (auto i = this->begin(); i != this->end(); i++) {
        // Implement this function for Lab 11, Checkpoint 1
        unsigned int hash_value = m_hash(*i);
        unsigned int index = hash_value % new_size;
        enlarge[index].push_back(*i);
      }

      m_table = enlarge;
    }
};
#endif //INC_10TH_DS_HASHSET_H
