//
// Created by luox6 on 2019/3/19.
//

#ifndef INC_9TH_DS_SET_H
#define INC_9TH_DS_SET_H
...
private:
    ...
    TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
      ...
      if (old_root != nullptr) {
        TreeNode<T> * tmp = new TreeNode<T>(old_root->value);
    ...
    iterator find(const T& key_value, TreeNode<T>* p) {
      TreeNode<T> * q = p; // Really necessary?
      while (q != NULL) {
        if (q->value > key_value) {
          q = q->left;
        } else if (q->value < key_value)
          q = q->right;
        else
          return iterator(q);
      }
