...
public:
    // default constructor, copy constructor, assignment operator, & destructor
    dslist() : head_(NULL), tail_(NULL), dummy(new Node<T>()), size_(0) {}
    dslist(const dslist<T>& old) { copy_list(old); }
    dslist& operator= (const dslist<T>& old);
    ~dslist() { destroy_list(); delete dummy; }
    ...
    iterator begin() { return head_ != NULL ? iterator(head_) : end(); }
...
template <class T>
void dslist<T>::memory_clear(Node<T>* node) {
  if (node != tail_) {
