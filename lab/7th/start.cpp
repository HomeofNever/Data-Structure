...
bool point_exist(vector<Point> const &points, Point const &p)
{
...
void print_grid(vector<vector<bool> > const& blocked_grid, vector<vector<Point>> const &path, int start_x, int start_y) {

  cout << "Here is the grid with the origin in the upper left corner, x increasing " << std::endl
       << "horizontally and y increasing down the screen.  An 'X' represents a blocked" << std::endl
       << "location and the 'S' represents the starting location." << std::endl
       << "$ represents the path" << std::endl;
  if (path.size() <= 0)
  {
    std::cout << "No Available Path" << std::endl;
    return;
  }
  vector<Point> p(path[0]);
  std::cout << "Path: ";
  ...
  for (int y=0; y<blocked_grid[0].size(); ++y) {
    for (int x=0; x<blocked_grid.size(); ++x) {
      if (x == start_x && y == start_y)
        ...
        cout << " X";
      else if (point_exist(p, Point(x, y)))
        cout << " $";
      else
        cout << " .";
    }
...
void one_step(const Point &p, vector<Point> &current, vector<vector<Point>> &path, vector<vector<bool>> & blocked_grid)
{
  if (p.x == 0 && p.y == 0)
  {
    current.push_back(p);
    ...
    {
      vector<Point> c(current);
      c.push_back(p);
      one_step(Point(p.x - 1, p.y), c, path, blocked_grid);
      one_step(Point(p.x, p.y - 1), c, path, blocked_grid);
    }
  }
}
...
int main(int argc, char* argv[]) {
  ...
  one_step(Point(start_x, start_y), current, path, blocked_grid);

  std::cout << "Possible Paths: " << path.size() << std::endl;
  std::cout << std::endl;
