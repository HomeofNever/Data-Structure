#ifndef INC_4TH_VEC_H
#define INC_4TH_VEC_H
...
{
    std::cout << "size: " << m_size << '|' << " allocated: " << m_alloc << std::endl;
    for (size_type i = 0; i < m_size; i++)
    {
        std::cout << m_data[i] << ' ';
    }
...
template <class T> unsigned int remove_matching_element(Vec<T> &v, const T &t)
{
    unsigned int num = 0;
    Vec<T> result;
